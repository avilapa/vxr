...
namespace vxr
{   
  void RidgidNoiseFilter::init(ref_ptr<NoiseSettings> noiseSettings, uint32 layer_id)
  {
    NoiseFilter::init(noiseSettings, layer_id);
    s_ = (RidgidNoiseSettings*)settings.get();
    ui_s_ = (RidgidNoiseSettings*)uiSettings.get();
  }

  float RidgidNoiseFilter::evaluate(vec3 point)
  {
    float noiseValue = 0;
    ...
    for (int i = 0; i < s_->numLayers; i++)
    {
      float v = 1 - glm::abs(noise->evaluate(point * frequency + s_->center));
      v *= v;
      v *= weight;
      weight = glm::clamp(v * s_->weightMultiplier, 0.0f, 1.0f);
  ...
  void RidgidNoiseFilter::onGUI()
  {
    ...
    if (node_open)
    {
      ...
      ImGui::DragFloat(label("Weight Multiplier").c_str(), &ui_s_->weightMultiplier, 0.01f, 0.01f, 10.0f);
      ImGui::TreePop();
    }
    ImGui::Spacing();
  }

  bool RidgidNoiseSettings::equals(ref_ptr<NoiseSettings> o)
  {
    ref_ptr<RidgidNoiseSettings> other = (RidgidNoiseSettings*)o.get();
  ...
  void RidgidNoiseSettings::assign(ref_ptr<NoiseSettings> o)
  {
    ref_ptr<RidgidNoiseSettings> other = (RidgidNoiseSettings*)o.get();
    SimpleNoiseSettings::assign(o);
    this->strength = other->strength;
    this->numLayers = other->numLayers;
    this->baseRoughness = other->baseRoughness;
    this->roughness = other->roughness;
